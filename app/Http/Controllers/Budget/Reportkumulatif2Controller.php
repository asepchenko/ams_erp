...
class Reportkumulatif2Controller extends Controller
{
    public function index(Request $request)
    {
        $title = "Report Summary Kumulatif";
        ...
        return view('budget.reportkumulatif2.index', compact('title','nik','name','group','tahun','data_group','kodegroup','start_period','end_period'));
    }

    public function searchData($start_period, $end_period, $tahun, $group)
    {
        // abort_unless(\Gate::allows('approval_report_document'), 403);
        $title = "Report Summary Periode ".strtoupper($start_period)." s/d ".strtoupper($end_period)." Tahun ".$tahun;
        ...
        $query = "select kode_group, sum(jum_budget) as budget, sum(progress) as progress, sum(jum_real) as realisasi, sum(jum_sisa) as sisa,
         (sum(jum_real) / sum(jum_budget)) * 100 as persen from budget.dbo.v_report_kumulatifperiode 
         where valid_to >= '".$awal."' and valid_to <= '".$akhir."' ".$where." group by kode_group order by 1 asc";
        //dd($query);
        $datanya = DB::select($query);

        return view('budget.reportkumulatif2.index', compact('title','name','datanya','kodegroup','tahun','data_group','start_period','end_period'));
    }
}
