...
class RealisasiDocumentController extends Controller
{
    public function index(Request $request)
    {
        ...
        $status = (!empty($_GET["status"])) ? ($_GET["status"]) : ("proses_payment");
        ...
        if ($request->ajax()) {
            ...
            if ($status == 'proses_payment') {
                $where .= ' and A.last_status in (\'' . $status . '\') ';
            } else {
                $where .= ' and A.last_status in (\'closed\') ';
            }
            ...
            //     and isnull(C.status_realisasi,\'0\')='.$status.' order by A.document_id desc');
            $data = DB::select('select A.*, format(A.tanggal_bayar,\'dd-MMM-yyyy\') as tglbayar,
            case when is_pu = 1 then \'Yes\' else \'No\' end as is_pu from APPROVAL.dbo.v_documents A 
            where 1=1 ' . $where . ' order by A.tglbuat asc');

            return DataTables::of($data)
                ->addColumn('action', function ($data) {
                    $button = '';
                    if ($data->last_status == "proses_payment") {
                        $button .= '<a href="realisasi-document/' . $data->id . '/edit">Edit</a>';
                    }
    ...
    public function edit($id)
    {
        ...
        $temp = DB::select('select id, nik, nama, kode_departemen, keterangan, last_status, created_at, created_by, no_document
    ...
    public function realisasiDocument(Request $request)
    {
        ...
        try {
            $temp = DB::select('finance.dbo.sp_realisasi_document_dev \'' . $id . '\',\'' . $tgl_realisasi . '\',\'' . $nama_file . '\',\'' . $nik . '\'');
            //$temp = DB::select('finance.dbo.sp_realisasi_document_dev \''.$id.'\',\''.$tgl_realisasi.'\',\''.$nama_file.'\',\''.$nik.'\'');
            $data = $temp[0];
            if ($data->hasil == "ok") {
                if ($request->file) {
