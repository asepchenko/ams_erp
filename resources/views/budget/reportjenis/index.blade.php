...
<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-8">
        <h1 class="m-0 text-dark">Report :  {{$periode}} - {{ $tahun }}</h1>
      </div><!-- /.col -->
      <div class="col-sm-4">
      ...
      <div id="collapseOne" class="collapse show" aria-labelledby="headingOne" data-parent="#accordion">
        <div class="card-body">
            ...
            <div class ="row">        
                ...
                <form class="form-horizontal">
                    <div class="form-group row">
                        <label for="coa" class="col-sm-4 col-form-label">COA Group</label>
                        <div class="col-sm-8">
                        <select class="form-control" name="coa" id="coa">
                            <option value="pilih">Pilih</option>
                            @foreach($data_coa as $datacoa)
                                <option value="{{ $datacoa->coagroup }}" {{ ($coapilih == $datacoa->coagroup) ? 'selected' : ''}}>{{ $datacoa->deskripsi }}</option>
                            @endforeach
                        </select>
                        </div>
                    </div>
                    ...
                    <div class="table-responsive">
                        ...
                        <thead>
                            ...
                            <tbody>
                                ...
                                @endforeach
                                @if ($total_budget == 0 or $total_pakai == 0)
                                    @php $totalpersen = 0; @endphp
                                @else                                    
                                    @php $totalpersen =  ($total_pakai / $total_budget) * 100;  @endphp
                                @endif
                            </tbody>
                            <tfoot>
                            <tr>
...
$(document).ready(function(){
    ...
    $('#btnFiterSubmitSearch').click(function(){
        ...
        var coa = $("#coa option:selected").val();
        var group = $("#group option:selected").val();
        var tahun = $("#tahun option:selected").val();
        spinner.show();
        window.location.href = "{{ url('budget/reportjenis') }}"+ "/search/" + coa + "/" + periode + "/" + tahun + "/" + group + "";
    });
